...
class TestMultiStrategy(CtaTemplate):
    """"""
    author = "用Python的交易员"
    ...
    def __init__(self, cta_engine, strategy_name, vt_symbol, setting):
        """"""
        super(TestMultiStrategy, self).__init__(
    ...
    def on_bar(self, bar: BarData):
        ...
        if (EngineType.BACKTESTING == self.get_engine_type()):
            self.write_log("[on_bar] : market prepare  open" )

            self.write_log("[on_bars] : market  open")
            ...
            while(tradeTime.__le__(end_date)):
                self.write_log("[on_bar] :      per munitue : " + tradeTime.__str__())
            ...
            self.write_log("[on_bar] : market prepare close" )
            self.write_log("[on_bar] : market  close" )

            pass


        self.write_log("on_bar : " + bar.datetime.__str__())
