...
def _init_BarV2_Model(db: Database):

    class DbBarV2Data(Model):
        """
        Candlestick bar data for database storage.
        ...
        @staticmethod
        def save_all(objs: List["DbBarV2Data"]):
            """
            save a list of objects, update if exists.
            """
            dicts = [i.to_dict() for i in objs]
            with db.atomic():
                for c in chunked(dicts, 50):
                    DbBarV2Data.insert_many(
                        c).on_conflict_replace().execute()

    barModel = DbBarV2Data
    if db.is_closed():
        db.connect()
    db.create_tables([DbBarV2Data])
    return barModel


class BarV2Storage(BarStorage):
